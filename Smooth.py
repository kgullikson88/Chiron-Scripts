...
def OptimalSmooth(order, normalize=True):
    ...
    training = DataStructures.xypoint(x=X_train[0][sorter_train], y=y_train[sorter_train], err=X_train[1][sorter_train],
                                      cont=X_train[2][sorter_train])
    validation = DataStructures.xypoint(x=X_test[0][sorter_test], y=y_test[sorter_test], err=X_test[1][sorter_test],
                                        cont=X_test[2][sorter_test])
...
def CrossValidation(order, smoothorder=5, lowreject=3, highreject=3, numiters=10, normalize=True):
    ...
    for i, windowsize in enumerate(windowsizes):
        ...
        if npixels > training.size:
            ...
            break
        smoothed = FittingUtilities.Iterative_SV(training.y.copy(), npixels, smoothorder, lowreject, highreject,
                                                 numiters)
